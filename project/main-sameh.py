...
training = False  # Set to True to train the model, False to evaluate

TRAINED_MODELS_DIR = "output"

DATASETS_DIR = "temporary"
VOC_DATASET_PATH = f"{DATASETS_DIR}_voc"
...
train_img_dir = os.path.join(base_dir, DATASETS_DIR, "train_annotated")
ann_path = os.path.join(base_dir, DATASETS_DIR, "train_annotated", "_annotations.coco.json")
...
base_dir = os.path.dirname(os.path.abspath(__file__))
test_image_dir = os.path.join(base_dir, DATASETS_DIR, "test")
...
if training:  
    ...
    model = LightFasterRCNNMobileNetV3(num_classes=num_classes)
    epochs = 150
    checkpoint_path = os.path.join(base_dir,"output", "MobilenetV3.pth")
...
def predict_unlabeled_images_and_save(model, dataset, class_id_to_name, device, reference_df, output_csv="mobilenet_submission.csv", score_threshold=0.7):
    ...
    # ✅ Rename 'Creme Brulee' column to 'Crème brulée' if needed
    df.rename(columns={"Creme Brulee": "Crème brulée"}, inplace=True)

    df.to_csv(output_csv, index=False)
    print(f"✅ Submission saved to {output_csv} (with 'Crème brulée' properly formatted)")
...
base_dir = os.path.dirname(os.path.abspath(__file__))
reference_path = os.path.join(base_dir, DATASETS_DIR, "final_submission.csv")
